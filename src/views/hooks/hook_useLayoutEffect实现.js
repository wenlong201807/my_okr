...
function useEffect(callback, dependencies) {
  if (hookStates[hookIndex]) {
    ...
    // hookStates[hookIndex] = dependencies;
    // callback();//  独立使用 useEffect时使用
    setTimeout(callback); // 配合使用useLayoutEffect时使用
  }
}
// 自定义useLayoutEffect
function useLayoutEffect(callback, dependencies) {
  if (hookStates[hookIndex]) {
    ...
    if (same) {
      ...
      hookStates[hookIndex++] = dependencies; // 可以使用
      // 添加一个微任务，在浏览器渲染之前执行
      queueMicrotask(callback); // queueMicrotask是浏览器自带的方法
    }
  } else {
    // 说明是第一次渲染
    hookStates[hookIndex++] = dependencies;
...
const Animation = () => {
  const red = React.useRef(); // {current:null} // 默认值
